...
def version_achive_list(cat_name,context,card,**_):
    ...
    for vercard in cards:
        if vercard.get('lack'):
            code += components['VersionLinks/Lack'].toxaml(card=vercard)
        else:
            code += components['VersionLinks/Common'].toxaml(card=vercard)
    code += '</StackPanel>'
    return code
...
def version_latest_list(context,**_):
    ...
    for version_type in ['release','snapshot']:
        if latest_version := mcv.get_latest(version_type):
            ...
            code += components['VersionLinks/Latest'].toxaml(card=vercard)
    return code